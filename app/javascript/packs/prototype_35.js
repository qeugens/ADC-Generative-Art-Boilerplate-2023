...
function createSwatch(containerSwatch) {
  ...
  const top = getRandomArbitrary(50, 414)
...
function mixing1() {
  let first_questions = [
    ...
    'будешь отвлекаться, когда мы вместе?',
...
function renderQuestionary() {
  const anketa = document.getElementById('prototype_35')
  ...
  mainText.innerText = 'wanna be my friend, huh?'
  const text = document.createElement('p')
  text.classList.add('text')
  text.innerText = 'honest answers required*'
  const special = document.createElement('button')
  special.classList.add('special')
  special.innerText = 'special for'
  const specialLine = document.createElement('div')
  specialLine.classList.add('specialLine')
  const description = document.createElement('p')
  description.classList.add('description')
  description.innerText =
    'подбери именно те вопросы, которые ты так хочешь задать своему другу, но боишься'
  ...
  leftSide.appendChild(mainText)
  leftSide.appendChild(text)
  leftSide.appendChild(special)
  leftSide.appendChild(specialLine)
  leftSide.appendChild(description)
  leftSide.appendChild(saveButton)
